...
std::ostream &operator<<(std::ostream &os, const Server &server) {
	...
	if (server._errorPages.size() > 0) {
		std::map<std::string, std::string>::const_iterator errorPage =
server._errorPages.begin(); 		std::map<std::string, std::string>::const_iterator
errorPageEnd = server._errorPages.end(); 		os<< INF_ALIGN0 << "Error pages:" <<
std::endl; 		while (errorPage != errorPageEnd) { 			os << INF_ALIGN1 << errorPage->first <<
": \"" << errorPage->second << "\"" << std::endl; 			errorPage++;
		}
	}
	...
	if (server._locations.size() > 0) {
		...
		while (location != lastLocation)
		{
			...
			os << INF_ALIGN1 << "Match" << ": \"" << location->getMatch() << "\"" <<
std::endl; 			os << INF_ALIGN1 << "Exact match" << ": \"" << (location->getExactMatch() ?
"yes" : "no") << "\"" << std::endl; 			os << INF_ALIGN1 << "Index" << ": ";
			...
			if (location->getErrorPages().size() > 0) {
				std::map<std::string, std::string> locationErrorPages =
location->getErrorPages(); 				std::map<std::string, std::string>::const_iterator errorPage
= locationErrorPages.begin(); 				std::map<std::string, std::string>::const_iterator
errorPageEnd = locationErrorPages.end(); 				os<< INF_ALIGN1 << "Error pages: (" <<
locationErrorPages.size() << ")" << std::endl; 				while (errorPage != errorPageEnd) { 					os
<< INF_ALIGN2 << errorPage->first << ": \"" << errorPage->second << "\"" << std::endl;
					errorPage++;
				}
			}
